...
$TotalRec = mysql_result(mysql_query("SELECT COUNT(*) FROM deploy"), 0);

$select = "SELECT * FROM deploy LIMIT $start, $perpage";
...
{
  echo '<a href='."./viewdeployequipment.php?id=" . ($start - $perpage) . '>'."Previous".'</a>';
}?>
...
{
  echo '<a href='."./viewdeployequipment.php?id=" . ($start + $perpage) . '>'."Next".'</a>';
}
