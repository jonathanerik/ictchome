			...
			<table class="status_table" border=1>
				...
				<?php
					...
					if(mysqli_num_rows($result)>0)
					{
						...
						{
							$status=$row['status'];
							if(($status!='PENDING') || ($status!='Request for Cancellation'))
							{
								$size='14';
							}
							else
							...
							else
							{
								if ($status == 'Request for Cancellation')
								{
									...
									<form action="view_requests.php" method="POST">
										...
										<input type="hidden" name="status_selected" value="<?php echo $status ?>">;
										<td colspan='2'><input type='submit' id='received' name='approve' value='Approve'></td>
									</fomr><?php
								}
								else
								{?>
									<form action="view_requests.php" method="POST">
										<td><a href="#loginScreen"><input type="text" class="edit" value="Edit"></a>
										<div id="loginScreen">
											<b>Edit Request</b>
											<a href="" class="cancel_close">&times;</a>
											<table align="center" class="register_table">
												<hr class="hr_popup"><br>
												<tr>
													<td><label>Comment:</label></td>
													<td><input type="text" name="comment" class="text_box" placeholder="Add comments." required/></td>
												</tr>
												<tr>
													<input type="hidden" name="request_selected" value="<?php echo $request_ID ?>">
													<input type="hidden" name="status_selected" value="<?php echo $status ?>">
													<td><input type='submit' id='received' name='approve' value='Approve'></td>
													<td><input type='submit' id='received' name='deny' value='Disapprove'></td>
												</tr>
											</table>
										</div></td>
										<div id="cover" >
						...
						if(isset($_POST['approve']))
							{
								...
								if(mysqli_num_rows($result_query)>0)
								{
									...
									if($status=='APPROVED')
									{
										$comment_update_r="UPDATE request SET comment ='$comment' WHERE request_ID= '$request_selected'";
										$update_success_r=mysqli_query($connect,$comment_update_r);

										$comment_update_f="UPDATE for_approval SET comment ='$comment' WHERE request_ID= '$request_selected'";
										$update_success_f=mysqli_query($connect,$comment_update_f);
													
										if(($query_update) && ($query_update_apprv) && ($update_success_r) && ($update_success_f))
										{
