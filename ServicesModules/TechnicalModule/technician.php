...
<div class="main-body">
 ...
 <center><div class="collapse navbar-collapse" id="navbarSupportedContent">
  ...
  <li class="nav-item active"> 
     ...
     <li class="nav-item active">
       <a class="nav-link" href="logout.php">Log Out<span></span><span></span></a>
     </li>
...
if(isset($_POST['Approved']))
{
	...
	$query_select="SELECT * FROM forms WHERE formid = '$formid'";
								...
								{
									$query_req="UPDATE request SET status ='Approved' WHERE formid = '$formid'";
...
if(isset($_POST['Completed']))
{
	...
	$query="UPDATE forms set status='Completed' where formid='$_POST[formid]'";
	
	$res=mysql_query($query);
	
	if($res){
		$_SESSION['success']="Request Fulfilled!";
	...
	$query_select="SELECT * FROM forms WHERE formid = '$formid'";
								...
								if(mysql_num_rows($result_query)>0)
								{
									$query_req="UPDATE request SET status ='Compeleted' WHERE formid = '$formid'";
									$query_update=mysql_query($query_req);

									$query_apprv="UPDATE forms SET status ='Completed' WHERE formid = '$formid'";
									$query_update_apprv=mysql_query($query_apprv);
								}
...
if(isset($_POST['Completed1']))
{
	...
	$query="UPDATE form1 set status='Completed' where formsid='$_POST[formsid]'";
	...
	$query_select="SELECT * FROM form1 WHERE formsid = '$formsid'";
								$result_query=mysql_query($query_select);
								if(mysql_num_rows($result_query)>0)
								{
									$query_req="UPDATE form1 SET status ='Compeleted' WHERE formsid = '$formsid'";
									$query_update=mysql_query($query_req);

									$query_apprv="UPDATE form1 SET status ='Completed' WHERE formsid = '$formsid'";
									$query_update_apprv=mysql_query($query_apprv);
								}
...
<table class="table table-striped table-hover ">
  ...
  <?php 
  $i=1;
  $query="Select * from `forms` WHERE status!='Rejected'";
	$res=mysql_query( $query);
	  ...
	  <td><?php echo $row['contactnumber'];?></td>
	    <td><?php echo $row['department'];?> </td>
		  <td><?php echo $row['building'];?></td>
			...
			<td> 
			<button type="submit" name="Completed" class="btn btn-info">Compeleted</button>
			</br></br>
 ...
 <?php 
  $i=1;
  $query="Select * from `form1` WHERE status!='Rejected'";
	$res=mysql_query( $query);
	$count=mysql_num_rows($res);
	if($count>0)
	{
	  ...
	  <td><?php echo $row['contactnumber'];?></td>
	    <td><?php echo $row['department'];?> </td>
		  <td><?php echo $row['building'];?></td>
			...
			<input type="hidden" name="formsid" value="<?php echo $row['formsid'];?>">
			<td> 
			<button type="submit" name="Completed1" class="btn btn-info">Compeleted</button>
			</br></br>
