<?php 
	...
	{
		if($_SESSION["user"]["userType"] == "Superior")
		{
	...
	<table border=3 width=500px height=100px>
		<form action="request_details_post.php?id=<?php echo $_GET['id']?>" method="POST">
			...
			<tr>
				...
				<td><textarea rows='2' cols='30' name="request" disabled/><?php echo $request['request_message']?></textarea></td>
			</tr><br>
			<tr>
				<td><label>Comments (ICTC):</label></td>
				<td><textarea rows='2' cols='30' name="comments_ictc" disabled/><?php echo $request['comments_ictc']?></textarea></td>
			</tr><br>
			<tr>
				<td><label>Comments (Superior):</label></td>
				<td><textarea rows='2' cols='30' name="comments" placeholder="" /><?php echo $request['comments_superior']?></textarea></td>
			</tr><br>	
			<tr>
				<td colspan='2'>
					<br>
						<center>
							<?php if($request['approved_ictc'] == false && $request['approved_superior']){?>
							...
							<?php }else{ ?>
								<button type="submit" name="status" value="Update">Update Comment</button>
							<?php }?>
						</center>
					<br>
				</td>
